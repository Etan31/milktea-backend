...
if (fs.existsSync(serviceAccountPath)) {
  try {
    ...
    try {
      ...
      firebaseDB = null;
      console.warn("Firebase Admin init ok but realtime DB not available - continuing without it.");
    }
  } catch (e) {
    console.warn("Failed to parse firebase-admin-key.json - continuing without Firebase.");
  }
} else {
  console.warn("firebase-admin-key.json not found — running without Firebase Realtime DB.");
}
...
const ensureTables = async () => {
  try {
    ...
    console.log("✅ Database tables ready (users, admin_accounts, products, orders, done_orders, notifications)");
  } catch (err) {
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    if (!email || !password) return safeJsonError(res, 400, "Email & password required");

    const result = await pool.query("SELECT * FROM users WHERE email=$1 LIMIT 1", [email]);
    if (result.rows.length === 0) return safeJsonError(res, 404, "User not found");

    const user = result.rows[0];
    const match = await bcrypt.compare(password, user.password || "");
    if (!match) return safeJsonError(res, 401, "Wrong password");

    req.session.user = { id: user.id, uid: user.uid, fullname: user.fullname, email: user.email, role: user.role };
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (!fullname || !email || !password) return safeJsonError(res, 400, "fullname, email, password required");
    ...
    const r = await pool.query(
      ...
      [uid, fullname, username || null, email, phone || null, gender || null, hashed]
    );
...
app.post("/api/admin-uid-login", async (req, res) => {
  try {
    ...
    const r = await pool.query("SELECT * FROM admin_accounts WHERE uid=$1 LIMIT 1", [uid]);
...
app.get("/api/users", async (req, res) => {
  try {
    const r = await pool.query("SELECT id, uid, fullname, username, email, phone, gender, role, created_at FROM users ORDER BY id ASC");
...
app.get("/api/admins", async (req, res) => {
  try {
    const r = await pool.query("SELECT id, uid, fullname, username, email, phone, gender, role, created_at FROM admin_accounts ORDER BY id ASC");
...
app.post("/api/products", async (req, res) => {
  try {
    ...
    if (!name || price == null) return safeJsonError(res, 400, "name & price required");
...
app.post("/api/products/update", async (req, res) => {
  try {
    ...
    if (!product_id || !name || price == null) return safeJsonError(res, 400, "product_id, name & price required");
...
app.post("/api/users/update", async (req, res) => {
  try {
    ...
    if (fields.length === 0) return safeJsonError(res, 400, "No fields to update");
    values.push(uid);
    const q = `UPDATE users SET ${fields.join(", ")} WHERE uid=$${values.length} RETURNING *`;
...
app.post("/api/admins/update", async (req, res) => {
  try {
    ...
    if (fields.length === 0) return safeJsonError(res, 400, "No fields to update");
    values.push(uid);
    const q = `UPDATE admin_accounts SET ${fields.join(", ")} WHERE uid=$${values.length} RETURNING *`;
    const r = await pool.query(q, values);
    res.json({ success: true, admin: r.rows[0] });
  } catch (err) {
...
app.post("/api/orders/create", async (req, res) => {
  try {
    const { user_id, email, items, total_amount, payment_method, delivery_info } = req.body;
    const firebaseUidFromClient = user_id || null;
    let pgUserId = null;
    if (firebaseUidFromClient) {
      const u = await pool.query("SELECT id FROM users WHERE uid=$1 LIMIT 1", [firebaseUidFromClient]);
...
app.post("/api/orders/update-status", async (req, res) => {
  ...
  if (!order_id || !status) return safeJsonError(res, 400, "Missing order_id or status");
  ...
  try {
    ...
    // fetch the order
    const ordRes = await client.query("SELECT * FROM orders WHERE order_id=$1 LIMIT 1", [order_id]);
    ...
    // update status
    await client.query("UPDATE orders SET status=$1 WHERE order_id=$2", [status, order_id]);
    ...
    if (!userUid && order.user_id) {
      const u = await client.query("SELECT uid FROM users WHERE id=$1 LIMIT 1", [order.user_id]);
    ...
    if (firebaseDB && userUid) {
      try {
        ...
        await notifRef.push({ order_id, status, message: notifMsg, created_at: new Date().toISOString() });
      } catch (e) {
    ...
    if ((status || "").toLowerCase() === "delivered") {
      ...
      try { if (typeof safeItems === "string") safeItems = JSON.parse(safeItems); } catch (e) { safeItems = []; }
      try { if (typeof safeDelivery === "string") safeDelivery = JSON.parse(safeDelivery); } catch (e) { safeDelivery = {}; }
